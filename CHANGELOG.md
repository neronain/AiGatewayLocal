...
- **ปุ่ม "Check for updates" ในคอนโซล** (`POST /admin/version/check`, admin เท่านั้น) ·
  เครื่อง production เคยค้างอยู่ที่ 1.10.0 หลายสัปดาห์หลัง 1.12.1 ออกไปแล้ว เพราะไม่มี
  อะไรบนจอบอกเลยว่ามันตามหลังอยู่ · ลูกค้าติดตั้งแบบเครื่องใครเครื่องมัน ไม่มีใครไล่ดูให้
  · แผงยังบอก **คำสั่งอัปเดตที่ตรงกับรูปแบบการติดตั้งของเครื่องนั้น** (checkout /
  container / ก๊อปไฟล์) ซึ่งอ่านจากดิสก์ ไม่ต้องต่อเน็ต
- **ตรวจอย่างเดียว ไม่มีปุ่มอัปเดตให้** · ทางที่ใช้จริงบนเครื่อง production คือแบบก๊อปไฟล์
  ทับ ซึ่งขั้นตอนที่ปลอดภัยมีห้าขั้นและทุกขั้นมีอยู่เพราะเคยข้ามแล้วเครื่องล่ม ·
  ปุ่มที่สำเร็จบ้างไม่สำเร็จบ้างบนเครื่องลูกค้า แย่กว่าไม่มีปุ่ม

- **`/v1/embeddings` และ `/v1/rerank`** — ปิดรูในผลิตภัณฑ์คู่ของเราเอง · LMDS deploy
  โมเดล embedding/rerank ได้ ทดสอบได้ benchmark ได้ แต่ LiteGate ตั้งประตูหน้ามันไม่ได้
  ลูกค้าที่ทำ RAG จึงได้ยินเรื่องที่มีรูตรงกลาง: "ยกเว้น embedding ที่ต้องยิงตรง ไม่ผ่าน key
  ไม่ผ่าน quota ไม่มี usage log"
  · rerank ยึดรูปแบบ Cohere/Jina อย่างที่ vLLM เสิร์ฟจริง และเป็น **ทางผ่าน ไม่ใช่ตัวแปล**
  · เปิดแค่ `/v1/rerank` ไม่เปิด `/v2/rerank` เพราะ Cohere v2 คืนคนละรูป — รับ path v2
  แล้วตอบ body v1 คือโกหก
- **การนับ token ของสองเส้นทางนี้ไม่เหมือน chat** · rerank **คูณ query ด้วยจำนวนเอกสาร**
  เพราะ cross-encoder รัน (query, doc) ใหม่ทุกคู่ — query 200 token × 50 เอกสาร = 10,000
  ไม่ใช่ 200 (ตรงกับที่ vLLM รายงานเอง) · ส่ง token id มาแทนข้อความก็ถูกนับ ไม่งั้นใช้ GPU ฟรี
  · ตรวจ context **ทีละชิ้น ไม่ใช่ผลรวม** — batch ที่รวมกันเกิน window เป็นเรื่องปกติของ RAG
  เอาผลรวมไปตรวจคือปฏิเสธงานที่เครื่องรับไหว

- **เพดานจำนวนชิ้นต่อคำขอ** `GW_MAX_BATCH_ITEMS` (ค่าตั้งต้น 2048 เท่าที่ OpenAI ใช้กับ
  `/v1/embeddings` · `0` = ไม่จำกัด) · โควตาเป็นแบบตรวจก่อนแล้วค่อยบันทึก และเส้นทาง chat
  ถูกคุมขนาดคำขอเดียวโดยโครงสร้างด้วย `context_tokens` อยู่แล้ว แต่ batch ไม่มีอะไรคุม —
  **คำขอเดียวที่มีแสนเอกสารใช้โควตาทั้งเดือนหมดในนัดเดียว แล้วค่อยโดนกันที่คำขอถัดไป**
  ซึ่งสายไปแล้ว · จำกัดจำนวนชิ้นแทนการหักโควตาล่วงหน้า เพราะอย่างหลังต้องเปลี่ยนลายเซ็นของ
  `QuotaService.check` ซึ่งกระทบทุกเส้นทาง
...
- **การติดตั้งแบบ native ยังรัน 4 worker อยู่ดี ทั้งที่ 1.12.1 ประกาศว่าแก้แล้ว** ·
  ...
  ถูกอ่านโดย `app.main:run` เท่านั้น ซึ่งเป็น console script ที่ unit ไม่ได้เรียก
  (มันเรียก uvicorn ตรง ๆ) · **`max_concurrency: 1` ของทุก endpoint จึงยังเป็น 4 คำขอ
  พร้อมกันจริงที่ backend**
  · ซ้ำร้ายกว่านั้น `app/state.py` ตัดสินว่าจะเตือนจาก `settings.workers` (= 1)
  **คำเตือนจึงไม่ขึ้นบนเครื่องที่ต้องการมันที่สุด**
  · `docker/docker-compose.prod.yml` เจอบั๊กเดียวกันและแก้ไปแล้วพร้อมคอมเมนต์อธิบาย —
  ไม่มีใครย้อนกลับมาแก้ unit · Docker ปกติไม่กระทบเพราะชุดนั้นมี Redis ให้อยู่แล้ว
  · unit อ่าน `${GW_WORKERS}` แล้ว พร้อม `Environment=GW_WORKERS=1` เป็นค่าตั้งต้น
  (วางไว้ **ก่อน** `EnvironmentFile` เพื่อให้ `.env` ทับได้) เครื่องที่ติดตั้งไว้ก่อนหน้า
  และ `.env` ยังไม่มีบรรทัดนั้นจึงได้ 1 ตามที่ควรเป็น

- **ค่าประมาณ token ต่ำกว่าความจริงเท่าตัวสำหรับภาษาไทย** · `CHARS_PER_TOKEN = 3.2`
  ตัวเดียวถูกใช้กับทุกภาษา · วัดจริงกับ `qwen3-embedding-8b` แล้วพบว่าอัตราจริงคือ
  **อังกฤษ 4.86 · โค้ด 3.26 · ไทยปนอังกฤษ 2.43 · ไทยล้วน 1.89 · ตัวเลข/สัญลักษณ์ 1.16**
  อักขระต่อ token — ค่าเดียวครอบไม่ได้ และยอดรวมของชุดทดสอบออกมาเป็น **73% ของจริง**
  · แยกเป็นสามอัตราตามชนิดอักขระ (ละติน 4.0 · ตัวเลข/สัญลักษณ์ 1.0 · นอก ASCII 1.6)
  ได้ **107% ของจริง** · **ตั้งใจให้เกินเล็กน้อย** เพราะในงานโควตา การนับต่ำกว่าจริงคือ
  ช่องโหว่ ส่วนการนับเกินเล็กน้อยแค่เข้มเกินไปหน่อย — สองอย่างนี้ไม่เท่ากัน
  · กระทบเฉพาะตอน backend **ไม่** รายงาน usage กลับมา (ทาง `estimated`)
  · ด่านตรวจ context ต่อชิ้นของ `/v1/rerank` ใช้ตัวประมาณเดียวกันแล้ว ไม่งั้นด่านกับ
  ยอดที่บันทึกจะไม่ตรงกัน

- **ปุ่ม `Update now` ในคอนโซล** — รันขั้นตอนห้าขั้นเดียวกับที่เอกสารบอกให้ทำมือ
  (สำรอง → ตรวจ syntax → ติดตั้ง → `import app.main` → restart + ยืนยัน `/healthz`)
  พร้อม **กู้คืนอัตโนมัติ** ถ้าขั้นไหนล้ม
  · **เกตเวย์ไม่ได้สิทธิ์เพิ่มเลย**: มันรันเป็น `litegate` ที่ไม่มี sudo และ
  `/opt/litegate/app` เป็น read-only สำหรับตัวมันเอง (`ProtectSystem=strict`) —
  ปุ่มแค่เขียนไฟล์คำขอลง `data/` แล้ว systemd path unit สั่งงานที่ต้องใช้ root ให้
  · **ไม่ดาวน์โหลดโค้ดเอง** ต้นทางต้องอยู่บนเครื่องแล้ว (`GW_UPDATE_SOURCE`) —
  ไซต์ air-gapped ต้องอัปเดตได้ และเกตเวย์ที่ดึงโค้ดจากเน็ตมารันไม่ใช่ของที่ลูกค้า
  ตกลงเอาไปติดตั้ง · ปิดทั้งกลไกได้ด้วย `GW_NO_UPDATE_BUTTON=1`
  · เครื่องที่ยังไม่มีกลไกนี้จะไม่เห็นปุ่ม (`can_apply: false`) เห็นแต่คำสั่งให้คัดลอก

### หมายเหตุเรื่องความเป็นส่วนตัว

- คำขอออกนอกเครื่อง **เฉพาะตอนมีคนกดปุ่ม** เท่านั้น — ไม่มีตอนบูต ตอนโหลดหน้า
  หรือเป็น background task · มีเทสสแกนทั้ง `app/` ว่า `release.check(` ถูกเรียกจาก
  endpoint นั้นที่เดียว และเทสที่บูตแอปจริงแล้วโหลดหน้าเว็บโดยนับคำขอขาออกได้ศูนย์
- สิ่งที่ส่งออกคือ GET เปล่า ๆ ไม่มี query ไม่มี body ไม่มี header ที่บอกเวอร์ชัน
  ชื่อเครื่อง หรือสถิติใด ๆ · การเทียบเลขทำที่ฝั่งเรา GitHub จึงรู้แค่ว่ามีคนอ่าน
  release ของ repo สาธารณะ · **ที่นี่ยังไม่มี telemetry**
- เบราว์เซอร์ไม่ได้ยิง GitHub เอง (ถ้ายิง IP ของคนเปิดคอนโซลจะไปโผล่ข้างนอกทุกครั้ง)
  — เกตเวย์เป็นคนถามให้ และมีเทสบังคับว่าทุก `fetch` ใน `app.js` ชี้กลับมาที่เกตเวย์

## 1.12.1 — 2026-09-21

### แก้

- **การติดตั้งแบบ native oversubscribe backend ตั้งแต่วินาทีแรก** · `scripts/bootstrap.sh`
  เขียน `GW_WORKERS=4` คู่กับ `GW_REDIS_URL=` ว่าง — ตัวนับคำขอที่กำลังวิ่งอยู่แชร์ข้าม
  worker ได้ก็ต่อเมื่อมี Redis ดังนั้น `max_concurrency: 1` ที่ผู้ดูแลเขียนไว้จึงกลายเป็น
  **4 คำขอพร้อมกันจริงที่ backend** ซึ่งตรงข้ามกับสิ่งเดียวที่ค่านั้นมีไว้ทำ และเป็นค่าที่
  คนตั้งตอนโมเดลใหญ่รับได้ทีละคำขอพอดี · **Docker ตั้งถูกมาตลอด** (มี Redis ให้ในชุด)
  — ทางติดตั้งที่เราแนะนำเองต่างหากที่ผิด
  · bootstrap เขียน `GW_WORKERS=1` แล้ว · เกตเวย์เป็นงาน I/O ล้วน 1 worker รับได้เยอะ
  และ `max_concurrency` ที่ถูกต้องมีค่ามากกว่าความขนานที่ไม่มีใครขอ
- **เตือนตอนสตาร์ตเมื่อ `GW_WORKERS > 1` โดยไม่มี Redis** — เตือน ไม่ใช่ปฏิเสธไม่ให้ขึ้น
  เพราะคนที่ตั้งใจรันหลาย worker โดยไม่สนใจ `max_concurrency` มีจริง และเกตเวย์ที่ไม่ยอม
  ขึ้นตอนตี 3 แย่กว่าเกตเวย์ที่ขึ้นพร้อมบรรทัดที่บอกตรง ๆ ว่ากำลังเกิดอะไร
  · จำเป็นเพราะ **bootstrap ไม่เคยเขียนทับ `.env` ที่มีอยู่** — เครื่องที่ติดตั้งไปแล้ว
  ยังถือ `GW_WORKERS=4` อยู่ คำเตือนนี้คือทางเดียวที่เจ้าของเครื่องจะรู้
...
- **เทสตัวนับโควตายิง 60 writer พร้อมกันใส่ SQLite** แล้วแดงบน CI (`database is locked`)
  ทั้งที่เครื่องพัฒนาผ่านตลอด · SQLite เขียนได้ทีละคน การยิง 60 ตัวจึงเพิ่มแค่ *"และ SQLite
  ต้องรอดจากการแย่งเขียน"* ซึ่งไม่ใช่คุณสมบัติที่เทสนี้ทดสอบและขึ้นกับความเร็วดิสก์
  · เหลือ 12 ซึ่งยังจับ read-modify-write ได้ (เสียหายตั้งแต่สองตัวที่สลับกันทำงาน)
  · ความเป็น atomic ตัวจริงพิสูจน์ที่ SQL ที่ compile ออกมา และการเขียนพร้อมกันจริง
  พิสูจน์บน Postgres ซึ่งรับ writer พร้อมกันได้จริง

## 1.11.1 — 2026-09-21

### แก้

- **`config.VERSION` ไม่ตรงกับ `pyproject.toml`** — 1.11.0 ถูก tag ไปทั้งที่บัมพ์แค่ที่เดียว
  `test_the_version_has_one_source` จับได้บน CI ตามหน้าที่ของมัน
- **ตัวนับโควตายอมแพ้เมื่อ SQLite บอกว่าไฟล์ถูกล็อก** · WAL + `busy_timeout` ทำให้คนเขียน
  ที่มาทีหลัง *รอ* แทนที่จะแพ้ แต่รอจนหมดเวลายังเป็นไปได้เมื่อคนเขียนเยอะพร้อมกันบนดิสก์ช้า
  — เจอบน CI runner (60 การบวกพร้อมกัน → `database is locked`) ทั้งที่บนเครื่องพัฒนาผ่านตลอด
  · ยอมแพ้ตรงนั้นคือ**การบวกหายจริง** ซึ่งเป็นสิ่งเดียวกับที่ 1.11.0 เพิ่งแก้ · ลองใหม่แบบ
  ถอยเพิ่มทีละรอบ **เฉพาะข้อผิดพลาดที่เป็นล็อกจริง** — `OperationalError` อื่น (สคีมาผิด
  ดิสก์เต็ม) เด้งทันทีเหมือนเดิม ไม่งั้นบั๊กจริงถูกกลืนแล้วช้าลงสี่เท่า

## 1.11.0 — 2026-09-21

### PostgreSQL ใช้ได้จริง — โดยที่ SQLite ยังเป็นค่าเริ่มต้นและไม่เปลี่ยนอะไรเลย

`GW_DATABASE_URL=postgresql+asyncpg://...` ทำงานได้แล้ว สำหรับลูกค้าที่รันหลาย
instance หรือที่ต้องมี replication/backup ตามมาตรฐานองค์กร · เครื่องเดียวยังไม่ต้อง
ลง Postgres และไม่มีพฤติกรรมไหนขยับ

**สามจุดที่ผูกกับ SQLite อยู่จริง ๆ — ไล่แล้วเจอ ไม่ใช่เดาว่าไม่มี:**

- **`func.date(ts)` ได้คนละคำตอบ** · SQLite เก็บเวลาแบบ UTC ไม่มี tz แล้ว `date()`
  ตัดเป็นวัน UTC ตรง ๆ · Postgres เก็บเป็น timestamptz แล้ว `date()` แปลงตามค่า
  `TimeZone` ของ session ก่อน — รายงานรายวันของลูกค้าที่เซิร์ฟเวอร์ไม่ได้ตั้งเป็น UTC
  จะเลื่อนขอบวันเงียบ ๆ และไม่ตรงกับ `WHERE ts >= since` ที่คิดแบบ UTC อยู่แล้ว
  แก้ด้วย `app/db/dialect.py::utc_date` ที่ compile ต่างกันตาม dialect
- **DDL อัปเกรดสคีมาตอนสตาร์ตใช้ได้เฉพาะ SQLite** · มันต่อสตริงเอง จึงออก
  `ADD COLUMN x BOOLEAN DEFAULT 0` (Postgres ปฏิเสธ: boolean รับ integer default
  ไม่ได้) และ `ADD COLUMN window ...` โดยไม่ quote (`WINDOW` เป็นคำสงวนของ Postgres)
  **ทั้งสองอย่างทำให้เกตเวย์สตาร์ตไม่ขึ้นเลย ไม่ใช่แค่คอลัมน์นั้นหาย** · 18 คอลัมน์
  boolean และ `quota_policies.window` อยู่ในข่าย · ตอนนี้ให้ SQLAlchemy เป็นคนเขียน
  ทั้ง quote และค่า default
- **ตัวนับโควตาในฐานข้อมูลเป็น read-modify-write** · มันรอดมาได้เพราะ SQLite ให้
  เขียนทีละคน · Postgres รับการเขียนพร้อมกันได้จริง การบวกจึงหายจริง และหายมากขึ้น
  ตามจำนวน instance — ซึ่งคือเหตุผลเดียวที่ลูกค้าย้ายมา · เปลี่ยนเป็น
  `UPDATE ... SET requests = requests + :n` ก้อนเดียว ไม่มีไวยากรณ์เฉพาะ dialect

`asyncpg` ย้ายออกจาก dependency หลักไปเป็น extra `[postgres]` · ตั้ง URL เป็น
Postgres โดยยังไม่ได้ลง จะได้ข้อความที่บอกคำสั่งที่ต้องรัน ไม่ใช่ ModuleNotFoundError
ดิบ ๆ (และไม่มีรหัสผ่านของฐานข้อมูลอยู่ในข้อความนั้น)

เทสส่วนใหญ่ตรวจที่ SQL ที่ถูก compile ออกมา จึงรันได้โดยไม่ต้องมี Postgres จริง
ใครมีเซิร์ฟเวอร์และอยากรันของจริงด้วย ตั้ง `GW_TEST_POSTGRES_URL` · ไม่ตั้ง = ข้าม
พร้อมเหตุผล ไม่ใช่ล้ม

### orjson เป็นของเสริม — และงานที่บล็อก event loop ถูกย้ายเฉพาะจุดที่วัดแล้วคุ้ม

เกตเวย์ไม่ได้คิดอะไรเลย มันแปลง JSON แล้วส่งต่อ · งาน CPU ต่อคำขอเกือบทั้งหมดจึงเป็น
encode/decode JSON และมันอยู่บน event loop เดียวกับทุกสตรีมที่กำลังไหล

`litegate[speed]` ลง `orjson` · ไม่ลงก็ตกกลับไปใช้ `json` ของ stdlib โดย**ผลลัพธ์
เหมือนกันทุกไบต์** (เทสยืนยันทั้งสองทางกับคอร์ปัสเดียวกัน) · วัดได้:

| งาน | stdlib | orjson |
|---|---|---|
| encode คำตอบ 59 KB (ไทยปนอังกฤษ) | 49 µs | 2.0 µs |
| encode chunk ของสตรีม 193 B | 1.19 µs | 0.24 µs |
| decode คำตอบ 59 KB จาก bytes | 40 µs | 28 µs |

ใช้ที่: ตัว response ของทุก endpoint (`default_response_class`) · ทุก frame ของ SSE
ทั้งขาเข้าและขาออก · body ที่ backend ตอบกลับมา · response cache

**ย้ายไป `to_thread` สองจุด และมีเหตุผลเป็นตัวเลขทั้งคู่:**

- `hashlib.scrypt` ของการเข้าสู่ระบบคอนโซล — **41 ms ต่อครั้ง** บนเครื่องพัฒนา
  (ตั้งใจให้ ~100 ms บนเซิร์ฟเวอร์เล็ก ซึ่งถูกต้องแล้วสำหรับรหัสผ่าน) · ใครกดเข้าสู่
  ระบบ = ทุกสตรีมใน worker นั้นสะดุดพร้อมกัน · ย้ายแล้วได้ผลจริงเพราะ scrypt
  **ปล่อย GIL**: 4 ครั้งเรียงกัน 158 ms · 4 ครั้งพร้อมกันใน thread 53 ms
- รอบตรวจทะเบียนโมเดล (`RegistryStore._watch`) — วัดช่วงที่ event loop ไม่ได้รันเลย
  กับทะเบียน 63 โมเดล: **เรียกตรง ๆ 15.5 ms · ผ่าน to_thread 0.6 ms**
  และเปลี่ยนไปใช้ `yaml.CSafeLoader` ด้วย ซึ่งลดงานลงจาก 64 ms เหลือ 7.7 ms —
  ทำให้งานน้อยลงดีกว่าย้ายงานเท่าเดิมไปที่อื่น

**ที่วัดแล้วแล้วไม่ย้าย** (สำคัญพอ ๆ กัน): base64 decode ของรูปขนาดเพดาน 10 MB
= 9.4 ms แต่ `binascii` **ไม่ปล่อย GIL** (4 ครั้งเรียงกัน 37 ms · ใน thread 41 ms)
ย้ายไปแล้ว event loop ก็ยังไม่ได้รัน ได้มาแต่ค่าข้าม thread · การเขียนไฟล์ YAML ของ
คอนโซลก็เล็กและนาน ๆ ครั้ง ไม่คุ้มเช่นกัน

## 1.10.0 — 2026-09-20

### Dashboard ได้เมนูย่อย — และเจอบั๊กสิทธิ์ระหว่างทาง

Dashboard: My quota · Available models · Backend health

**บั๊กที่เกือบปล่อยไป:** `dash-health` ถูกแอปซ่อนตามสิทธิ์ด้วย
`$('health-wrap').hidden = !admin` · กลไกเมนูย่อยเดิมสลับหมวดด้วย `hidden` เหมือนกัน
แปลว่า **คนที่ไม่ใช่ admin กดปุ่ม "Backend health" แล้วแผงจะเปิดให้เห็น**

แยกเป็นคนละชั้นแล้ว: `hidden` = สิทธิ์ (แอปคุม) · `.subtab-off` = ผู้ใช้เลือกดูหมวดอื่นอยู่
(เมนูคุม) · เมนูย่อยไม่แตะ `hidden` อีกต่อไป

- หมวดที่ผู้ใช้ไม่มีสิทธิ์เห็น **ไม่มีปุ่มขึ้นในเมนู** เลย
- `buildSubtabs()` เรียกซ้ำได้ และถูกเรียกอีกครั้งหลังรู้สิทธิ์ เพราะตอน init ยังไม่รู้
- คีย์ที่จำไว้ชี้ไปหมวดที่ตอนนี้ไม่มีสิทธิ์ → ตกกลับหมวดแรกแทนที่จะเปิดของต้องห้าม
- เหลือหมวดเดียวหลังกรองสิทธิ์ → ไม่มีเมนู และคืนหมวดนั้นให้แสดงตามปกติ

ยืนยันในเบราว์เซอร์จริง: บังคับ `showSubtab(dashboard,'health')` ตอนไม่มีสิทธิ์
แล้วแผงยังปิดอยู่ · ปุ่ม 2 อันตอนไม่ใช่ admin · 3 อันตอนเป็น admin

ครบสี่แท็บที่ยาวที่สุด: Account (4) · Dashboard (3) · Access & keys (5) · Quota (3)

---

## 1.9.1 — 2026-09-20

- แท็บ **My Account** ได้เมนูย่อยด้วย: My quota · Models I can use · My keys · Password
  ไม่ต้องแก้โค้ดเพิ่มเลย ใส่ `data-subtabs` กับ `data-subtab-label` ตามกลไกกลางที่ 1.9.0
  วางไว้ — ซึ่งเป็นเหตุผลที่ทำเป็นกลไกกลางตั้งแต่แรกแทนที่จะแก้ทีละแท็บ

ตอนนี้สามแท็บที่ยาวที่สุดเป็นเมนูย่อยครบ: Account (4) · Access & keys (5) · Quota (3)
